...
const long disconnectedDuration = 2000; // milisecond
unsigned long previousDisconnectedMillis = 0; // last time when disconnected was found
  ...
  if (receiverStateVar == LOW) {
    ...
    if (TESTING_STATE == true) {
      ...
      if (Serial.available() > 0) {
        ...
        if (isBluetoothConnected == false and isSerialConnected == false) {
          blinkLED();
        } else if (currentMillis - previousDisconnectedMillis >= disconnectedDuration) {
          previousDisconnectedMillis = currentMillis;
          isBluetoothConnected = false;
          isSerialConnected = false;
        } else {
          turnOnLED(); // turn on the LED light, signaling either bluetooth or jetson module is connected
        }
      }
