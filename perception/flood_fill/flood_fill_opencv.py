...
def start():
    ...
    while True:
        ...
        # OpenCV FloodFill
        curr_img = xyz_norm.reshape((d1, d2, 3)).astype(np.float32)
        mask = np.zeros((curr_img.shape[0] + 2, curr_img.shape[1] + 2), np.uint8)
        seed_point = (d1 - 1, int(d2 / 2))
        _, retval, _, _ = cv2.floodFill(image=curr_img,
                                        seedPoint=seed_point,
                                        newVal=(0, 0, 0),
                                        loDiff=(0.15, 0.15, 0.15),
                                        upDiff=(0.15, 0.15, 0.15),
                                        mask=mask)
        
        norm_umatrix = cv2.resize(cv2.UMat(mask[1:-1, 1:-1]), (d2 * 4, d1 * 4))
